...
def solve_mvo_soft_constraint(alpha, B, F, D, lambda_risk=1.0, 
    ...
    Transaction cost formula (from optimizer.py):
      gamma * 0.5 * (|w_i - w_drift_i| + |w_i| - |w_drift_i|)
    ...
    try:
        ...
        z_start_idx = None
        use_z = apply_drift_penalty or (penalty_l1_w > 0)
        ...
        if apply_drift_penalty:
            ...
            tc_vec = gamma_arr * 0.5 * (drift_dev + np.abs(weights) - np.abs(w_drift_arr))
